...
public class UnidadeService {
    ...
    public UnidadeResponseDTO findById(Long id){
         ...
         Unidade buscaUnidade = unidadeRepository.findById(id).orElseThrow(() -> new RuntimeException("Unidade não encontrada!"));
         return unidadeMapper.toDTO(buscaUnidade);

    }

    public UnidadeResponseDTO atualizarDados(Long id, UnidadeRequestDTO dados){
      Unidade buscaUsuario = unidadeRepository.findById(id).orElseThrow(() -> new RuntimeException("Unidade não encontrada!"));
